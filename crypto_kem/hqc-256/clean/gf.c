 ...
 */
static uint16_t PQCLEAN_HQC256_CLEANgf_reduce(uint64_t x, size_t deg_x) {
 ...
 */
uint16_t PQCLEAN_HQC256_CLEANgf_mul(uint16_t a, uint16_t b) {
    uint8_t c[2] = {0};
    gf_carryless_mul(c, (uint8_t) a, (uint8_t) b);
    uint16_t tmp = c[0] ^ (c[1] << 8);
    return PQCLEAN_HQC256_CLEANgf_reduce(tmp, 2 * (PARAM_M - 1));
}
 ...
 */
uint16_t PQCLEAN_HQC256_CLEANgf_square(uint16_t a) {
    uint32_t b = a;
    uint32_t s = b & 1;
    ...
    return PQCLEAN_HQC256_CLEANgf_reduce(s, 2 * (PARAM_M - 1));
}
 ...
 * @param[in] a Element of GF(2^PARAM_M)
 */
uint16_t PQCLEAN_HQC256_CLEANgf_inverse(uint16_t a) {
    uint16_t inv = a;
    uint16_t tmp1, tmp2;

    inv = PQCLEAN_HQC256_CLEANgf_square(a); /* a^2 */
    tmp1 = PQCLEAN_HQC256_CLEANgf_mul(inv, a); /* a^3 */
    inv = PQCLEAN_HQC256_CLEANgf_square(inv); /* a^4 */
    tmp2 = PQCLEAN_HQC256_CLEANgf_mul(inv, tmp1); /* a^7 */
    tmp1 = PQCLEAN_HQC256_CLEANgf_mul(inv, tmp2); /* a^11 */
    inv = PQCLEAN_HQC256_CLEANgf_mul(tmp1, inv); /* a^15 */
    inv = PQCLEAN_HQC256_CLEANgf_square(inv); /* a^30 */
    inv = PQCLEAN_HQC256_CLEANgf_square(inv); /* a^60 */
    inv = PQCLEAN_HQC256_CLEANgf_square(inv); /* a^120 */
    inv = PQCLEAN_HQC256_CLEANgf_mul(inv, tmp2); /* a^127 */
    inv = PQCLEAN_HQC256_CLEANgf_square(inv); /* a^254 */
    return inv;
}
