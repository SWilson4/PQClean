 ...
 */
void PQCLEAN_HQC256_CLEANcode_encode(uint64_t *em, const uint8_t *m) {
    uint8_t tmp[VEC_N1_SIZE_BYTES] = {0};

    PQCLEAN_HQC256_CLEANreed_solomon_encode(tmp, m);
    PQCLEAN_HQC256_CLEANreed_muller_encode(em, tmp);
...
void PQCLEAN_HQC256_CLEANcode_decode(uint8_t *m, const uint64_t *em) {
    ...
    PQCLEAN_HQC256_CLEANreed_muller_decode(tmp, em);
    PQCLEAN_HQC256_CLEANreed_solomon_decode(m, tmp);
