...
void PQCLEAN_HQCRMRS256_CLEAN_hqc_pke_keygen(unsigned char *pk, unsigned char *sk) {
    ...
    PQCLEAN_HQCRMRS256_CLEAN_seedexpander_init(&sk_seedexpander, sk_seed, SEED_BYTES);

    randombytes(pk_seed, SEED_BYTES);
    PQCLEAN_HQCRMRS256_CLEAN_seedexpander_init(&pk_seedexpander, pk_seed, SEED_BYTES);
    ...
    PQCLEAN_HQCRMRS256_CLEAN_vect_mul(s, y, h);
...
void PQCLEAN_HQCRMRS256_CLEAN_hqc_pke_encrypt(uint64_t *u, uint64_t *v, uint8_t *m, unsigned char *theta, const unsigned char *pk) {
    ...
    PQCLEAN_HQCRMRS256_CLEAN_seedexpander_init(&seedexpander, theta, SEED_BYTES);
    ...
    PQCLEAN_HQCRMRS256_CLEAN_vect_set_random_fixed_weight(&seedexpander, r2, PARAM_OMEGA_R);
    PQCLEAN_HQCRMRS256_CLEAN_vect_set_random_fixed_weight(&seedexpander, e, PARAM_OMEGA_E);

    // Compute u = r1 + r2.h
    PQCLEAN_HQCRMRS256_CLEAN_vect_mul(u, r2, h);
    ...
    PQCLEAN_HQCRMRS256_CLEAN_vect_mul(tmp2, r2, s);
    ...
    PQCLEAN_HQCRMRS256_CLEAN_seedexpander_release(&seedexpander);
}
...
void PQCLEAN_HQCRMRS256_CLEAN_hqc_pke_decrypt(uint8_t *m, const uint64_t *u, const uint64_t *v, const unsigned char *sk) {
    ...
    // Compute v - u.y
    PQCLEAN_HQCRMRS256_CLEAN_vect_resize(tmp1, PARAM_N, v, PARAM_N1N2);
    PQCLEAN_HQCRMRS256_CLEAN_vect_mul(tmp2, y, u);
    PQCLEAN_HQCRMRS256_CLEAN_vect_add(tmp2, tmp1, tmp2, VEC_N_SIZE_64);
