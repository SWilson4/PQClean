 ...
 */
static uint16_t PQCLEAN_HQC128_CLEAN_gf_reduce(uint64_t x, size_t deg_x) {
...
uint16_t PQCLEAN_HQC128_CLEAN_gf_mul(uint16_t a, uint16_t b) {
    ...
    return PQCLEAN_HQC128_CLEAN_gf_reduce(tmp, 2 * (PARAM_M - 1));
}
...
uint16_t PQCLEAN_HQC128_CLEAN_gf_square(uint16_t a) {
    ...
    return PQCLEAN_HQC128_CLEAN_gf_reduce(s, 2 * (PARAM_M - 1));
}
