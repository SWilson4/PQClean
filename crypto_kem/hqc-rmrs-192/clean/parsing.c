...
void PQCLEAN_HQCRMRS192_CLEAN_store8(uint8_t *out, uint64_t in) {
    out[0] = (in >> 0x00) & 0xFF;
    out[1] = (in >> 0x08) & 0xFF;
    out[2] = (in >> 0x10) & 0xFF;
    out[3] = (in >> 0x18) & 0xFF;
...
uint64_t PQCLEAN_HQCRMRS192_CLEAN_load8(const uint8_t *in) {
    uint64_t ret = in[7];
 ...
 */
void PQCLEAN_HQCRMRS192_CLEAN_hqc_secret_key_from_string(uint64_t *x, uint64_t *y, uint8_t *pk, const uint8_t *sk) {
    seedexpander_state sk_seedexpander;

    PQCLEAN_HQCRMRS192_CLEAN_seedexpander_init(&sk_seedexpander, sk, SEED_BYTES);
    sk += SEED_BYTES;

    PQCLEAN_HQCRMRS192_CLEAN_vect_set_random_fixed_weight(&sk_seedexpander, x, PARAM_OMEGA);
    PQCLEAN_HQCRMRS192_CLEAN_vect_set_random_fixed_weight(&sk_seedexpander, y, PARAM_OMEGA);
    memcpy(pk, sk, PUBLIC_KEY_BYTES);

    PQCLEAN_HQCRMRS192_CLEAN_seedexpander_release(&sk_seedexpander);
}
...
void PQCLEAN_HQCRMRS192_CLEAN_hqc_public_key_to_string(uint8_t *pk, const uint8_t *pk_seed, const uint64_t *s) {
    ...
    pk += SEED_BYTES;
    PQCLEAN_HQCRMRS192_CLEAN_store8_arr(pk, VEC_N_SIZE_BYTES, s, VEC_N_SIZE_64);
}
...
void PQCLEAN_HQCRMRS192_CLEAN_hqc_public_key_from_string(uint64_t *h, uint64_t *s, const uint8_t *pk) {
    ...
    PQCLEAN_HQCRMRS192_CLEAN_seedexpander_init(&pk_seedexpander, pk, SEED_BYTES);
    PQCLEAN_HQCRMRS192_CLEAN_vect_set_random(&pk_seedexpander, h);

    PQCLEAN_HQCRMRS192_CLEAN_load8_arr(s, VEC_N_SIZE_64, pk + SEED_BYTES, VEC_N_SIZE_BYTES);

    PQCLEAN_HQCRMRS192_CLEAN_seedexpander_release(&pk_seedexpander);
}
 ...
 */
void PQCLEAN_HQCRMRS192_CLEAN_hqc_ciphertext_to_string(uint8_t *ct, const uint64_t *u, const uint64_t *v, const uint8_t *d, const uint8_t *salt) {
 ...
 */
void PQCLEAN_HQCRMRS192_CLEAN_hqc_ciphertext_from_string(uint64_t *u, uint64_t *v, uint8_t *d, uint8_t *salt, const uint8_t *ct) {
