...
int PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_enc(uint8_t *ct, uint8_t *ss, const uint8_t *pk) {
    ...
    PQCLEAN_HQCRMRS192_CLEAN_shake256_512_ds(&shake256state, theta, tmp, VEC_K_SIZE_BYTES + SEED_BYTES + SALT_SIZE_BYTES, G_FCT_DOMAIN);

    // Encrypting m
    PQCLEAN_HQCRMRS192_CLEAN_hqc_pke_encrypt(u, v, m, theta, pk);

    // Computing d
    PQCLEAN_HQCRMRS192_CLEAN_shake256_512_ds(&shake256state, d, m, VEC_K_SIZE_BYTES, H_FCT_DOMAIN);
    ...
    PQCLEAN_HQCRMRS192_CLEAN_shake256_512_ds(&shake256state, ss, mc, VEC_K_SIZE_BYTES + VEC_N_SIZE_BYTES + VEC_N1N2_SIZE_BYTES, K_FCT_DOMAIN);

    // Computing ciphertext
    PQCLEAN_HQCRMRS192_CLEAN_hqc_ciphertext_to_string(ct, u, v, d, salt);
...
int PQCLEAN_HQCRMRS192_CLEAN_crypto_kem_dec(uint8_t *ss, const uint8_t *ct, const uint8_t *sk) {
    ...
    PQCLEAN_HQCRMRS192_CLEAN_hqc_ciphertext_from_string(u, v, d, salt, ct);
    ...
    // Computing d'
    PQCLEAN_HQCRMRS192_CLEAN_shake256_512_ds(&shake256state, d2, m, VEC_K_SIZE_BYTES, H_FCT_DOMAIN);
    ...
    result |= PQCLEAN_HQCRMRS192_CLEAN_vect_compare(d, d2, SHAKE256_512_BYTES);
