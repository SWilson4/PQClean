 ...
 */
static uint16_t PQCLEAN_HQCRMRS128_CLEAN_gf_reduce(uint64_t x, size_t deg_x) {
...
uint16_t PQCLEAN_HQCRMRS128_CLEAN_gf_square(uint16_t a) {
    ...
    return PQCLEAN_HQCRMRS128_CLEAN_gf_reduce(s, 2 * (PARAM_M - 1));
}
...
uint16_t PQCLEAN_HQCRMRS128_CLEAN_gf_inverse(uint16_t a) {
    ...
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(a); /* a^2 */
    tmp1 = PQCLEAN_HQCRMRS128_CLEAN_gf_mul(inv, a); /* a^3 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(inv); /* a^4 */
    tmp2 = PQCLEAN_HQCRMRS128_CLEAN_gf_mul(inv, tmp1); /* a^7 */
    tmp1 = PQCLEAN_HQCRMRS128_CLEAN_gf_mul(inv, tmp2); /* a^11 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_mul(tmp1, inv); /* a^15 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(inv); /* a^30 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(inv); /* a^60 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(inv); /* a^120 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_mul(inv, tmp2); /* a^127 */
    inv = PQCLEAN_HQCRMRS128_CLEAN_gf_square(inv); /* a^254 */
    return inv;
}
